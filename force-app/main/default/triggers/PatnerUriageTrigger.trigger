trigger PatnerUriageTrigger on PatnerUriage__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    if(Utility_Common.isDataIkouProfile()){return;}

    if(!TriggerIsActive__c.getOrgDefaults().isActive__c){return;}
    Trg_PatnerUriage handler = new Trg_PatnerUriage(Trigger.isExecuting, Trigger.size);
